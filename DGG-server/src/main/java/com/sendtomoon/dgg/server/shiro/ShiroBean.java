...
public class ShiroBean {
	...
	public FilterRegistrationBean testFilterRegistration() {
		...
		registration.setFilter(new ShiroFilter());// 添加过滤器
		registration.addUrlPatterns("/*");// 设置过滤路径，/*所有路径
		registration.addInitParameter("targetFilterLifecycle", "true");// 添加默认参数
		registration.setName("shiroFilter");// 设置优先级
		registration.setOrder(20);// 设置优先级
